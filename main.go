...
func ProcessArgs() (*Template, error) {
	...
	for range only.Once {
		...
		help_all := flag.Bool("help-all", false, "Show all help.")
		help_variables := flag.Bool("help-variables", false, "Show variables help.")
		help_functions := flag.Bool("help-functions", false, "Show functions help.")
		help_examples := flag.Bool("help-examples", false, "Show examples help.")

		tmpl.jsonFile = flag.String("json", "", "JSON file for template reference.")
		tmpl.jsonString = flag.String("json-string", "", "JSON string for template reference.")

		tmpl.createFlag = flag.String("create", "", "Create file from template and cleanup after.")
		tmpl.templateFile = flag.String("template", "", "Source template file.")
		tmpl.outFile = flag.String("out", "", "Output file.")

		tmpl.templateString = flag.String("template-string", "", "Inline template string.")
		tmpl.removeFiles = flag.Bool("remove", false, "Remove template file after processing.")
		tmpl.overWrite = flag.Bool("force", false, "Force overwrite of output file.")

		tmpl.execShell = flag.Bool("shell", false, "Assume a shell script output and execute afterwards.")
		...
		if *help_all {
			...
			HelpExamples()
			os.Exit(0)
		} else if *help_variables {
			Help()
			HelpVariables()
			os.Exit(0)
		} else if *help_functions {
			Help()
			HelpFunctions()
			os.Exit(0)
		} else if *help_examples {
			Help()
			HelpExamples()
			os.Exit(0)
		}
